...
class Banae_Post_Title extends Widget_Base {
	...
	public function get_title() {
		return __( 'Post Title', 'banana-addons-for-elementor' );
	}
	...
	protected function register_controls() {
		...
		Banae_Post_Title::add_register_controls( $this );
	...
	protected function render() {
		...
		echo sprintf(
			'<%1$s class="banae-post-title">%2$s%3$s%4$s</%1$s>',
			esc_attr( $tag ),
			esc_attr( $link_open ),
			esc_html( $title ),
			esc_attr( $link_close )
